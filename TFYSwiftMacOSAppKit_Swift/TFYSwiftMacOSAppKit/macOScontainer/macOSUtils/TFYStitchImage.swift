...
public struct TFYStitchConfig {
    ...
    /// 边距
    public var contentInsets: NSEdgeInsets = NSEdgeInsets(top: 4, left: 4, bottom: 4, right: 4)
    /// 每页最大数量（分页显示时使用）
...
public struct TFYStitchResult {
    ...
    public init(images: [NSImage], isSuccess: Bool, error: Error? = nil, processingTime: TimeInterval = 0, cacheKey: String? = nil) {
        self.images = images
...
public class TFYStitchImage: NSObject {
    ...
    private class func generateCacheKey(
        ...
        return "stitch_\(imageHashes)_\(sizeString)_\(configHash)".stitchSha256
    }
    
    /// 获取缓存的图片
    private class func getCachedImage(for key: String) -> NSImage? {
    ...
    private class func stitchImage(
        ...
        let backgroundRect = NSRect(origin: .zero, size: NSSize(width: size.width, height: size.height))
        backgroundRect.fill()
...
public extension TFYStitchImage {
    ...
    class func createCircular(
        ...
        config.contentMode = .scaleProportionallyUpOrDown
        let result = stitchImagesSync(images: images, size: size, config: config)
        return result.images.first
    }
...
private extension TFYStitchImage {
    ...
    class func drawShadow(
        ...
        // 绘制阴影路径
        let shadowPath = NSBezierPath(rect: rect)
        shadowPath.fill()
...
public extension TFYStitchImage {
    ...
    class func createAnimatedStitch(
        ...
        animationOptions: CAMediaTimingFunctionName = .easeInEaseOut
    ) -> NSImage? {
        // 这里可以实现动画效果，暂时返回静态图片
...
private extension TFYStitchImage {
    ...
    /// 应用滤镜
    class func applyFilter(to image: NSImage, filter: CIFilter) -> NSImage? {
        ...
        resultImage.addRepresentation(NSBitmapImageRep(cgImage: cgImage))
        return resultImage
    }
    
    /// 优化图片尺寸
    class func optimizeImageSize(_ image: NSImage, targetSize: CGSize) -> NSImage {
        let imageRep = NSBitmapImageRep(
            bitmapDataPlanes: nil,
        ...
        guard let imageRep = imageRep else { return image }
        ...
        let resultImage = NSImage(size: NSSize(width: targetSize.width, height: targetSize.height))
        resultImage.addRepresentation(imageRep)
        return resultImage
    }
    
    /// 压缩图片质量
    class func compressImage(_ image: NSImage, quality: CGFloat) -> Data? {
        guard let tiffData = image.tiffRepresentation,
              let bitmapRep = NSBitmapImageRep(data: tiffData),
