...
public extension NSObject {
    ...
    class func exchangeClassMethodsForClass(_ targetClass: AnyClass, originalSelector: Selector, swizzledSelector: Selector) {
        ...
        guard let metaclass = object_getClass(targetClass) else { return }
        let swizzledImp = method_getImplementation(swizzledMethod)
        guard let swizzledEncoding = method_getTypeEncoding(swizzledMethod) else { return }
        let didAddMethod = class_addMethod(metaclass, originalSelector, swizzledImp, swizzledEncoding)
        if didAddMethod {
            let originalImp = method_getImplementation(originalMethod)
            guard let originalEncoding = method_getTypeEncoding(originalMethod) else { return }
            class_replaceMethod(metaclass, swizzledSelector, originalImp, originalEncoding)
        } else {
            method_exchangeImplementations(originalMethod, swizzledMethod)
        }
    }
